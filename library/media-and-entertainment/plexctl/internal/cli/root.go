...
type rootFlags struct {
	...
	// invocation. Mirrors the PLEXCTL_NO_LEARN env var.
	noLearn bool
...
Run 'plexctl-pp-cli doctor' to verify auth and connectivity.`,
	...
	rootCmd.SetVersionTemplate("plexctl-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select MediaContainer)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'plexctl-pp-cli profile list')")
	...
	rootCmd.AddCommand(newButlerCmd(flags))
	rootCmd.AddCommand(newDownloadQueueCmd(flags))
	rootCmd.AddCommand(newHubsCmd(flags))
	rootCmd.AddCommand(newLibraryCmd(flags))
	rootCmd.AddCommand(newLivetvCmd(flags))
	rootCmd.AddCommand(newLogCmd(flags))
	rootCmd.AddCommand(newMediaCmd(flags))
	rootCmd.AddCommand(newPlayQueuesCmd(flags))
	...
	rootCmd.AddCommand(newTvPlexProvidersEpgIdentifierDeviceIdCmd(flags))
	rootCmd.AddCommand(newUpdaterCmd(flags))
	...
	rootCmd.AddCommand(newIdentityPromotedCmd(flags))
	rootCmd.AddCommand(newPhotoPromotedCmd(flags))
