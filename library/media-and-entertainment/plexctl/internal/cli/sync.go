...
Resource scoping:
  ...
  plexctl-pp-cli sync --resources hubs,library

  # Full resync (ignore previous checkpoint)
  plexctl-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  plexctl-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  plexctl-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  plexctl-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "plexctl-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "download-queue":
		return db.UpsertDownloadQueue(data)
	case "add":
		return db.UpsertAdd(data)
	case "item":
		return db.UpsertItem(data)
	case "download_queue_items":
		return db.UpsertDownloadQueueItems(data)
	case "hubs":
		return db.UpsertHubs(data)
	case "livetv":
		return db.UpsertLivetv(data)
	case "media":
		return db.UpsertMedia(data)
	case "play-queues":
		return db.UpsertPlayQueues(data)
	case "play_queues_items":
		return db.UpsertPlayQueuesItems(data)
	case "reset":
		return db.UpsertReset(data)
	case "shuffle":
		return db.UpsertShuffle(data)
	case "unshuffle":
		return db.UpsertUnshuffle(data)
	case "playlists":
		return db.UpsertPlaylists(data)
	case "generators":
		return db.UpsertGenerators(data)
	case "playlists_items":
		return db.UpsertPlaylistsItems(data)
	case "security":
	...
	case "tv-plex-providers-epg-identifier-device-id":
		return db.UpsertTvPlexProvidersEpgIdentifierDeviceId(data)
	case "updater":
		return db.UpsertUpdater(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"hubs":    "/hubs/search/voice",
		"library": "/library/all",
		"status":  "/status/sessions/history/all",
	}
...
var reconcileTypedTables = map[string]string{
	...
	"play_queues_items":    "play_queues_items",
	"playlists":            "playlists",
	...
	"shuffle":              "shuffle",
	"status":               "status",
	"tv-plex-providers-epg-identifier-device-id": "tv_plex_providers_epg_identifier_device_id",
	"unshuffle": "unshuffle",
	"updater":   "updater",
}
